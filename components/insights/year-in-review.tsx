...
export function YearInReview() {
  ...
  const yearStats = useMemo(() => {
    ...
    const totalSpent = year2025Transactions.reduce((sum, t) => sum + t.amount, 0);
    
    // Top recipient
    const recipientMap = new Map<string, { amount: number; count: number }>();
    year2025Transactions.forEach((t: Transaction) => {
      if (t.recipient) {
    ...
    const firstTransaction = new Date(Math.min(...year2025Transactions.map((t: Transaction) => t.date || t.createdAt)));
    const lastTransaction = new Date(Math.max(...year2025Transactions.map((t: Transaction) => t.date || t.createdAt)));
    ...
    return {
      ...
      debtsCleared: debts.filter((d: { currentBalance: number }) => d.currentBalance === 0).length,
    };
      ...
      <Card>
        <CardContent className="p-12 text-center">
          ...
          <p className="text-muted-foreground">
            Add transactions from 2025 to see your year-in-review insights.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-background rounded-lg">
              ...
              <p className="text-2xl font-bold text-primary">{formatAmount(yearStats.totalSpent)}</p>
            </div>
            <div className="text-center p-4 bg-background rounded-lg">
              <p className="text-sm text-muted-foreground mb-1">Transactions</p>
              <p className="text-2xl font-bold">{yearStats.totalTransactions}</p>
            </div>
            <div className="text-center p-4 bg-background rounded-lg">
              <p className="text-sm text-muted-foreground mb-1">Avg Transaction</p>
              <p className="text-2xl font-bold text-green-500">{formatAmount(yearStats.avgTransaction)}</p>
            </div>
            <div className="text-center p-4 bg-background rounded-lg">
              <p className="text-sm text-muted-foreground mb-1">Recipients</p>
              <p className="text-2xl font-bold text-blue-500">{yearStats.totalRecipients}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center p-6 bg-yellow-500/10 rounded-lg">
              <p className="text-xl font-bold mb-2">{yearStats.topRecipient.name}</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center p-6 bg-red-500/10 rounded-lg">
              <p className="text-xl font-bold mb-2">{yearStats.mostExpensiveMonth.month}</p>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-muted rounded-lg">
              <p className="text-3xl font-bold mb-1">{yearStats.totalCategories}</p>
              <p className="text-xs text-muted-foreground">Categories Tracked</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <p className="text-3xl font-bold mb-1">{yearStats.totalBudgets}</p>
              <p className="text-xs text-muted-foreground">Budgets Created</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <p className="text-3xl font-bold mb-1">{yearStats.debtsCleared}</p>
              <p className="text-xs text-muted-foreground">Debts Cleared</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <p className="text-3xl font-bold mb-1">{yearStats.totalRecipients}</p>
