...
type ManualTx = Transaction & { mpesaReference?: string };
type FlaggedDuplicate = { parsed: ParsedTx; matches: ManualTx[] };
type FlaggedAction = 'add' | 'merge' | 'ignore';

function fuzzyMatchManualTransaction(parsed: ParsedTx, manualTx: ManualTx): boolean {
  // Strict: match by Mpesa reference if present
  if (manualTx.mpesaReference && parsed.reference) {
    return manualTx.mpesaReference.trim().toUpperCase() === parsed.reference.trim().toUpperCase();
  }
  ...
  const dateMatch = Math.abs(new Date(parsed.date).getTime() - new Date(manualTx.date).getTime()) < 2 * 24 * 60 * 60 * 1000;
  const rec1 = (parsed.recipient || "").toLowerCase();
  const rec2 = (manualTx.recipient || "").toLowerCase();
  const recipientMatch = !!rec1 && !!rec2 && (rec1.includes(rec2) || rec2.includes(rec1));
...
export default function AddTransactionForm() {
  ...
  // Parse and preview transactions when messages change, flag possible duplicates
  ...
  useMemo(() => {
    ...
    for (const message of messageLines) {
      try {
        ...
        // Check for possible duplicate with manual transactions
        const manualMatches = existingTransactions.filter(
          (tx) => tx.rawMessage?.startsWith("Manual entry") && fuzzyMatchManualTransaction(previewTx, tx)
        );
    ...
    setFlaggedActions(flagged.reduce<Record<number, FlaggedAction>>((acc, _, i) => { acc[i] = 'add'; return acc; }, {}));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      flaggedDuplicates.forEach((dup, i) => {
        if (flaggedActions[i] === 'ignore') flaggedToSkip.add(dup.parsed.rawMessage);
        if (flaggedActions[i] === 'merge') flaggedToMerge.push({ parsed: dup.parsed, matches: dup.matches });
      });
      ...
      for (const message of messageLines) {
        ...
        try {
          ...
          const mergeIdx = flaggedToMerge.findIndex(f => f.parsed.rawMessage === message);
          if (mergeIdx !== -1) {
            const match = flaggedToMerge[mergeIdx].matches[0];
            await db.transact(
              db.tx.transactions[match.id]
                .update({
              ...
              <div className="rounded-lg border p-4 space-y-2">
                ...
                {flaggedDuplicates.length > 0 && (
                  <div className="bg-yellow-100 border border-yellow-300 rounded p-2 mb-2 text-yellow-900 text-xs">
                    ...
                    <ul className="list-disc ml-4">
                      {flaggedDuplicates.map((dup, i) => (
                        <li key={i} className="mb-2">
                          <div>
                            <span>Imported: <b>{dup.parsed.recipient}</b> Ksh {dup.parsed.amount} on {new Date(dup.parsed.date).toLocaleDateString("en-KE")}<br/></span>
                            <span>Matches manual: {dup.matches.map(m => `${m.recipient} Ksh ${m.amount} (${new Date(m.date).toLocaleDateString("en-KE")})`).join(", ")}</span>
                          </div>
                          <div className="flex gap-2 mt-1">
                            <label>
                              <input
                                ...
                                onChange={() => setFlaggedActions(a => ({ ...a, [i]: 'add' }))}
                              /> Add as new
                            </label>
                            <label>
                              <input
                                ...
                                onChange={() => setFlaggedActions(a => ({ ...a, [i]: 'merge' }))}
                              /> Merge with manual
                            </label>
                            <label>
                              <input
                                ...
                                onChange={() => setFlaggedActions(a => ({ ...a, [i]: 'ignore' }))}
                              /> Ignore
                            </label>
                          </div>
                        </li>
                    ...
                    <span>Choose what to do with each duplicate before adding.</span>
                  </div>
                )}
